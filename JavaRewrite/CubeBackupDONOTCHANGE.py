...
class RubiksCube:
    ...
    def edge_orientation(self):
        ...
        for (face1, pos1), (face2, pos2) in self.edge_positions:
            color1 = getattr(self, face1)[pos1[0]][pos1[1]]
            color2 = getattr(self, face2)[pos2[0]][pos2[1]]

            if face1 == "whiteface" or face1 == "yellowface":
                if color1 == ("w" if face1 == "whiteface" else "y"):
                    orientations.append(0)
                else:
                    orientations.append(1)

            elif face2 == "whiteface" or face2 == "yellowface":
                if color2 == ("w" if face2 == "whiteface" else "y"):
                    orientations.append(0)
                else:
                    orientations.append(1)
            else:
...
def pruning_table():
    ...
    moves = ["r", "rprime", "l", "lprime", "f", "fprime",
             "u", "uprime", "b", "bprime", "d", "dprime",
             "rtwo", "ltwo", "ftwo", "utwo", "btwo", "dtwo"]
    ...
    while queue:
        ...
        for move in moves:
            ...
            index = new_cube.edge_orientation_index()
            table_sum+=index
            print("index:")

            if pruning_table[index] == -1:
                pruning_table[index] = depth + 1
...
count = 0
for i, v in enumerate(pruning_table):
    if v != -1:
        count += 1
print("Total unique edge orientations visited:", count)
print(cube.edge_orientation_index())
print(pruning_table )
phase1_solution = phase1(cube, pruning_table, 7)
